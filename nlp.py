...
def pro(word,flag):
	global pos,neg,voc,db,tot_freq_pos,tot_freq_neg,voc_size
	db1 = neg
	tot = tot_freq_neg                                         #probility formula:
	if flag==1:                                                #p(w/c)=count(w,c)+1/count(c)+|v|
		db1 = pos                                              #this formula converted to logarithem(log)
		tot = tot_freq_pos                                     #log p(w/c)=log(count(w,c))-log(count(c)+|v|)
	if word in db1:                                            #pro method develeped by using this formula
...
def verify():
	...
	for i in range(1,11):
		...
		for j in range(1,11):
			if j != i:
				testpos = open(str(j)+'out_pos2.txt')	
				traindat1 += testpos.read()
				testpos.close()
				testneg = open(str(j)+'out_neg2.txt')
		...
		fp,fn,tp,tn = 0,0,0,0
		tes = open(str(i)+'out_pos2.txt','r').read().split('\n')
		for doc in tes:
			if predict(doc)==1:
				tp += 1
			else:
				fn += 1
		testl = open(str(i)+'out_neg2.txt','r').readlines()
